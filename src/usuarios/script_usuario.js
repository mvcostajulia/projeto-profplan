async function listar(usuarios) {
    usuarios.sort((a, b) => a.nome.localeCompare(b.nome));
    console.log(usuarios)
    const tbody = document.getElementById('usuarios').querySelector('tbody');
    tbody.innerHTML = ''; 
    usuarios.forEach(usuario => {
        ...
        tr.innerHTML = `
            <td class="buttons">
                            <button title="Editar" onclick="editarUsuario('${usuario._id.toString()}')" ${confirmedActions}>
                            ...
                            <button title="Visualizar" onclick="visualizarUsuario('${usuario._id.toString()}')">
...
async function listarUsuarios() {
    ...
    usuarios.sort((a, b) => a.nome.trim().localeCompare(b.nome.trim()));
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const realizarPesquisa = async () => {
        ...
        try {
            const response = await fetch(`/usuarios/gerenciar/pesquisa/${termo}`);
            if (!response.ok) {
                tbody.innerHTML = '';
            }
            const resultados = await response.json();
            resultados.sort((a, b) => a.nome.localeCompare(b.nome));
            sessionStorage.setItem('resultadosUsuarios', JSON.stringify(resultados));
            tbody.innerHTML = '';
            listar(resultados);
        } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const svgFiltro = document.getElementById('filtro');
    function abrirFiltros(){
        ...
        botaoFiltrar.addEventListener("submit", async function(event) {
            ...
            tbody.innerHTML = '';
            try {
                const response = await fetch(`/usuarios/gerenciar/filtrar?${params.toString()}`);
                if (!response.ok) {
                    tbody.innerHTML = '';
                }
                const resultados = await response.json();
...
function editarUsuario(id) {
    const url = `/usuarios/gerenciar/pesquisa/${id}`;
    fetch(url)
        .then(response => {
            if (!response.ok) {
        ...
        .catch(error => {
            ...
            alert('Ocorreu um erro ao buscar os usuarios');
    });
}
...
function visualizarUsuario(id) {
    ...
    fetch(url)
        ...
        .then(data => {
            localStorage.setItem('usuarioData', JSON.stringify(data));
            window.location.href='/usuarios/ver'
        })
        .catch(error => {
            console.error('Erro:', error);
            alert('Ocorreu um erro ao buscar os usuarios');
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    botaoDownload.addEventListener("click", async function(event) {
        const usuarios=JSON.parse(sessionStorage.getItem('resultadosUsuarios'));
        let csvContent = "Nome,Matricula,Área, E-mail,Função,Status\n";
        usuarios.forEach(usuario =>{
            const linha = [
                usuario.nome || '',
                usuario.matricula || '',
