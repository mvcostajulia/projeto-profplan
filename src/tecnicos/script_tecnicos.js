async function listar(tecnicos) {
    const tbody = document.getElementById('tecnicos').querySelector('tbody');
    tbody.innerHTML = ''; 
    ...
    tecnicos.forEach(tecnico => {
        ...
        tr.innerHTML = `
            <td class="buttons">
                            <button title="Editar" onclick="editarTecnico('${tecnico._id.toString()}','${tecnico.matricula}','${tecnico.area}')" ${confirmedActions}>
                            ...
                            <button title="Visualizar" onclick="visualizarTecnico('${tecnico._id.toString()}')">
...
async function listarTecnicos() {
    ...
    tecnicos.sort((a, b) => a.nome.trim().localeCompare(b.nome.trim()));
    sessionStorage.setItem('resultadosTecnicos', JSON.stringify(tecnicos));
    listar(tecnicos);
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const realizarPesquisa = async () => {
        ...
        try {
            const response = await fetch(`/tecnicos/gerenciar/pesquisa/${termo}`);
            if (!response.ok) {
                tbody.innerHTML = '';
            }
            const resultados = await response.json();
            resultados.sort((a, b) => a.nome.localeCompare(b.nome));
            sessionStorage.setItem('resultadosTecnicos', JSON.stringify(resultados));
            tbody.innerHTML = '';
            listar(resultados);
        } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function abrirFiltros(){
        ...
        <div class="filtragem">
            <form id="form-filtrar">
                ...
                <select id="area" name="area">   
                    ...
                    <option value="Apoio Educação">Apoio Educação</option>
                    <option value="Automotiva">Automotiva</option>                                                      
                    <option value="Construção Civil">Construção Civil</option>
                    <option value="Eletroeletrônica">Eletroeletrônica</option>
                    <option value="Gestão">Gestão</option>
                    <option value="Madeira e Mobiliária">Madeira e Mobiliária</option>
        ...
        botaoFiltrar.addEventListener("submit", async function(event) {
            ...
            tbody.innerHTML = '';
            try {
                const response = await fetch(`/tecnicos/gerenciar/filtrar?${params.toString()}`);
                if (!response.ok) {
                    tbody.innerHTML = '';
                }
                const resultados = await response.json();
...
async function editarTecnico(id, matricula, area) {
    coletaAgendamentos(id);
    try {
        ...
        const matriculaLogada = data.usuario.matricula;
        const funcaoLogada = data.usuario.funcao;
        const areaLogada = data.usuario.area;
        if(matricula===matriculaLogada || funcaoLogada==="Administrador" || (areaLogada===area && funcaoLogada==="Facilitador")){
            const url = `/tecnicos/gerenciar/pesquisa/${id}`;
            fetch(url)
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Falha ao buscar tecnicos');
                    }
                    return response.json();
                    
                })
                .then(data => {
                    localStorage.setItem('tecnicoData', JSON.stringify(data));
                    window.location.href='/tecnicos/editar'
                })
                .catch(error => {
                    console.error('Erro:', error);
                    alert('Ocorreu um erro ao buscar os tecnicos');
            });
        }else{
            exibirNotificacao("Você não possui permissão de edição para este técnico");
        }
    } catch (error) {
        console.error(error);
    }
...
async function coletaAgendamentos(id) {
    const url = `/agendamentos/gerenciar/coleta/${id}`;
    fetch(url)
        .then(response => {
            if (!response.ok) {
                throw new Error('Falha ao buscar tecnicos');
        ...
        .then(data => {
            sessionStorage.setItem('agendamentoTecnico', JSON.stringify(data));
        })
        .catch(error => {
            console.error('Erro:', error);
    });
...
function visualizarTecnico(id) {
    ...
    fetch(url)
        ...
        .then(data => {
            localStorage.setItem('tecnicoData', JSON.stringify(data));
            window.location.href='/tecnicos/ver'
        })
        .catch(error => {
            console.error('Erro:', error);
            alert('Ocorreu um erro ao buscar os tecnicos');
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    botaoDownload.addEventListener("click", async function(event) {
        const tecnicos=JSON.parse(sessionStorage.getItem('resultadosTecnicos'));
        let csvContent = "Nome,Matricula,Area,Turno,Facilitador,Status\n";
        tecnicos.forEach(tecnico =>{
            const linha = [
                tecnico.nome || '',
                tecnico.matricula || '',
